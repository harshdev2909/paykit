...
import { Wallet, CheckoutSession, Merchant } from "../database/models";
...
export async function refreshWalletPublicKeys(): Promise<void> {
  const [wallets, checkouts] = await Promise.all([
...
function normalizeCheckoutAsset(code: string): "XLM" | "USDC" | "PYUSD" {
  const u = code.toUpperCase();
  if (u === "XLM" || u === "USDC" || u === "PYUSD") return u;
  throw new Error(`Unsupported checkout asset for settlement: ${code}`);
}
...
export async function handlePaymentRecord(record: HorizonPaymentRecord): Promise<void> {
  ...
  if (!isOurWallet(from) && !isOurWallet(to) && !toIsCheckout) {
    return;
  }
  ...
  await TxModel.findOneAndUpdate(
    { txHash },
    {
      $set: {
        ...
        toWallet: to,
        assetCode,
        assetIssuer: record.asset_issuer ?? record.source_asset_issuer ?? undefined,
        amount,
        status: "success",
        updatedAt: new Date(),
      },
  ...
  if (toIsCheckout) {
    ...
    if (session && session.asset === assetCode && parseFloat(amount) >= parseFloat(session.amount)) {
      ...
      try {
        ...
        if (settlementWallet) {
          const normalized = normalizeCheckoutAsset(assetCode);
          const doPay = async () => {
          ...
          try {
            ...
            if (isNoTrust && (normalized === "USDC" || normalized === "PYUSD")) {
        ...
        queueWebhookEvent(
          "checkout.failed",
          { checkoutSessionId: session._id.toString(), error: String(err) },
          merchantDoc?.webhookUrl
        ).catch(() => {});
      }
    }
  }
