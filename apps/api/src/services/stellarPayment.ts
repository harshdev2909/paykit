/**
 * Internal Stellar payment submission (custodial wallets). Not exposed as a public HTTP route.
 */
import {
  TransactionBuilder,
  Operation,
...
export interface SubmitWalletPaymentParams {
  fromWalletId: string;
  toAddress: string;
...
export interface SubmitWalletPaymentResult {
  txHash: string;
}

export async function submitWalletPayment(req: SubmitWalletPaymentParams): Promise<SubmitWalletPaymentResult> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      import("./webhookService").then(({ queueWebhookEvent }) => {
        queueWebhookEvent("payment.completed", {
          txHash,
          fromWallet: wallet.publicKey,
          toWallet: req.toAddress,
          asset: req.asset,
